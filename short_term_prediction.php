...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['exercise_name'])) {
    ...
    $custom_time_frame = filter_var($_POST['custom_time_frame'] ?? 7, FILTER_VALIDATE_INT, ["options" => ["min_range" => 1, "max_range" => 60]]) ?: 7;
...
//Prediction function for short-term (up to 2 months)
function predictWeight($data, $days_ahead, $common_sets = 4, $common_reps = 12) {
    ...
    //Step 1: Calculate trend-based improvement rate using linear regression on recent 30 days
    $last_date = strtotime(end($data)['log_date']);
    if ($last_date === false) {
        return ['error' => 'Invalid last date in data'];
    }
    $cutoff_date = $last_date - (30 * 24 * 60 * 60); // Last 30 days
    ...
    foreach ($data as $index => $entry) {
        ...
        $volume = $weight * ($entry['reps'] ?: 1) * ($entry['sets'] ?: 1); //Default to 1 if reps/sets missing
    ...
    // Step 4: Predict using the trend-based rate
    $rate_predicted_weight = $last_weight + $avg_daily_rate * $days_ahead;

    // Step 5: Combine predictions (favor rate-based if trend is strong)
    ...
    return [
        ...
        'volume_adjustment' => 1 // Default value to avoid undefined key warning
    ];
}
...
<head>
    ...
    <title>Fitness Tracker | Short-Term Prediction</title>
    ...
    <nav class="bg-gray-800 bg-opacity-90 backdrop-blur-md shadow-lg fixed w-full top-0 z-50">
        <div class="max-w-6xl mx-auto px-4 flex justify-between items-center py-4 relative">
            ...
            <div class="hidden md:flex space-x-8">
                ...
                <a href="long_term_prediction.php" class="nav-link"><i class="ri-line-chart-line text-lg"></i> Long-Term Prediction</a>
            ...
            <div id="mobile-menu" class="absolute top-16 right-4 bg-gray-800 bg-opacity-95 backdrop-blur-md rounded-lg w-48 shadow-xl hidden">
                ...
                <a href="long_term_prediction.php" class="mobile-nav-link"><i class="ri-line-chart-line text-lg"></i> Long-Term Prediction</a>
    ...
    <main class="max-w-6xl mx-auto px-4 mt-24 animate-fade-slide">
        <div class="bg-gray-800 rounded-lg p-6 shadow-lg">
            <div class="text-center mb-6">
                <h2 class="text-3xl font-bold text-blue-500">Short-Term Exercise Prediction</h2>
                <p class="text-gray-400">Predict your performance in the next 1–60 days</p>
            </div>
            ...
            <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    <label for="custom_time_frame" class="block text-lg">Prediction Time Frame (Days, 1–60):</label>
                    <input type="number" name="custom_time_frame" id="custom_time_frame" class="mt-2 p-2 rounded bg-gray-700 text-white w-full" min="1" max="60" value="<?= $custom_time_frame ?: 7 ?>" required />
                </div>
